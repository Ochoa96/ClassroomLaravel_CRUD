...
<html>
    ...
    <body>
        <h1 style="text-align: center; color: #4c4c4c;">Create new <span style="color: #96a2f3">Student</span></h1>
        <div style="text-align: center; margin-bottom: 20px;">
            <a href="{{ route('students.index') }}">Go Back to List</a>
        </div>       
        <div class="container">
            <form action="{{ route('students.store') }}" method="POST" style="display: grid;">
                @csrf
                ...
                @enderror
                <label>Age:</label>
                <input type="text" name="age" placeholder="Your age" value="{{ old('age') }}">
                @error('age')
                    <br>
                    <p class="error-message">{{ $message }}</p>
                    <br>
